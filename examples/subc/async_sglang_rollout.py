...
class TaskNoToolLV4(BaseModel):
    ...
    subtasks: Optional[List[TaskNoToolLV5]] = Field(
        default=None,   # allow the field to be omitted / None
        max_items=5     # if it’s given, list length ≤ 5
    )
    conclusion: str
    title: str
...
class AsyncSGLangRollout(BaseRollout):
    ...
    def _init_sampling_params(self, **kwargs):
        ...
        self.sampling_params['json_schema'] = json.dumps(
                SolutionNoTool.model_json_schema()
            ),
    ...
    def update_sampling_params(self, **kwargs):
        ...
        for key, value in old_sampling_params_args.items():
            self.sampling_params[key] = value
        if 'json_schema' not in self.sampling_params:
            self.sampling_params['json_schema'] = json.dumps(SolutionNoTool.model_json_schema())
    ...
    def execute_method(self, method: Union[str, bytes], *args, **kwargs):
        if method == "chat_completion":
            ...
            sampling_params_dict = {
                ...
                "json_schema": json.dumps(SolutionNoTool.model_json_schema())
            }
            output = None
            if self._tp_rank == 0:
                loop = asyncio.get_event_loop()
                print(sampling_params_dict)
                output = loop.run_until_complete(
                    self._engine.async_generate(
                        prompt=prompt_str,
